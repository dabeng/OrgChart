...
(function($) {
  ...
  $.fn.jOrgChart = function(options) {
    ...
    // ajust org-chart accoring to users' own ideas
    if (opts.draggable) {
      $chart.draggable({ cancel: 'div.node' });
    }
    if (opts.create) {
      opts.create();
    }
  ...
  function hideSiblings($node, justSiblings) {
    ...
    $upperLink = $nodeContainer.parent().prev().prev();
  ...
  function createNode(nodeData, opts) {
    ...
    $nodeDiv = $('<div>', {'id': nodeData[opts.nodeID]})
      .addClass('node')
    ...
    $nodeDiv.on('mouseenter mouseleave', function(event) {
      var $node = $(this);
      $edge = $node.children('.edge');
      $topEdge = $node.children('.topEdge');
      $rightEdge = $node.children('.rightEdge');
      $bottomEdge = $node.children('.bottomEdge');
      $leftEdge = $node.children('.leftEdge');
      var temp;
